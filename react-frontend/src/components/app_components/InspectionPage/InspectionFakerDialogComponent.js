...
import inspectionFakerFactory from "./inspectionFakerFactory";
...
const InspectionFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [highwayIds, sethighwayIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("highways")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        sethighwayIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = inspectionFakerFactory(props.user,fakerCount,highwayIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("inspection").create(elem));
...
export default connect(mapState, mapDispatch)(InspectionFakerDialogComponent);
